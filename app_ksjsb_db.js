...
let ck = "";
let usre_name;
let codeArr = ['2771222874395655','2771294317056588','2771282441544165','2771304230195913','2771323229770126']
...
async function user_info(num) {
	...
	if (result.result == 1) {
		...
		console.log(`\n用户${num}: ${result.data.userData.nickname} 🎉 `);
        console.log(`账户余额: ${result.data.totalCash} 元,总额：${result.data.allCash} 元，金币: ${result.data.totalCoin} 枚\n`);
		usre_name = result.data.userData.nickname;

		msg += `\n\n用户${num}:  ${result.data.userData.nickname} 🎉\n 账户余额: ${result.data.totalCash} 元,总额：${result.data.allCash} 元,金币: ${result.data.totalCoin}  枚 \n`;
...
async function sign_info() {
	...
	if (result.data.nebulaSignInPopup.todaySigned == false) {
		...
		msg += `签到信息: ${usre_name} 今天未签到,去签到喽!\n`;
		await $.wait(3 * 1000);
		await signin();
	} else if (result.data.nebulaSignInPopup.todaySigned == true) {
		console.log(`\n 签到信息: ${usre_name} 今天已签到,明天再来吧!\n`);
		msg += `签到信息: ${usre_name} 今天已签到\n`;
	} else {
		console.log(`\n 签到信息: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `签到信息: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}


//查询是否领取新手红包
async function task_info() {
	let url = {
		url: `https://nebula.kuaishou.com/rest/n/nebula/activity/earn/overview/tasks?addressBookAccessStatus=false&pushNotificationStatus=false&imei=&idfa=&oaid=a8767d1c248740e`,
		headers: {
			// "Host": "nebula.kuaishou.com",
			'Cookie': ck[0],
		},
	};
	let result = await httpGet(url, `查询新手红包`);
    //console.log(result.data.nebulaGoldenAreaTask)
    if (result.data.nebulaGoldenAreaTask) {
        result = result.data.nebulaGoldenAreaTask
        console.log(`当前是新手任务第 ${result.currentDay} 天`)
        console.log(`今日新手红包状态：${result.linkText}`)
        msg += `今日新手红包状态：${result.linkText}\n`
        if (result.linkText == '立即领取') {
            console.log('领取中')
            await $.wait(1000)
            await task_sign()
            isNotify = true
        } else if (result.linkText == '去看视频') {
            console.log(`请先去app看一秒视频`)
            msg += `请先去app看一秒视频\n`
            isNotify = true
        }
    }
...
async function task_sign() {
	...
	let result = await httpGet(url, `领取新手红包`);
    //console.log(result)
    if (result.result == 1) {
        result = result.data
        if (result.linkText == '提醒我来领') {
            console.log(`领取成功`)
            msg += `今日新手红包领取成功\n`
        } else {
            console.log(`领取出错了`)
...
async function zzz_help(code) {
	let url = {
        ...
        body: '{\"assistanceId\":\"' + code + '\"}'
	};
    //console.log(url.headers.Cookie)
	let result = await httpPost(url, `周周赚助力`);
    //console.log(result)
    if (result) {
        console.log(`为账号 ${result.assistanceInfo.createNickName} 助力结果：${result.msg}\n`)
        msg += `为账号 ${result.assistanceInfo.createNickName} 助力结果：${result.msg}\n`
        isNotify = true
        if (result.msg.indexOf('每天只能给好友助力一次') != -1) {
            //jixu = false
            console.log('本账号停止助力')
            return false
        }
...
async function box_info() {
	...
	if (result.result == 1) {
		if (result.data.openTime == -1) {
			...
			msg += `宝箱信息: 宝箱冷却中, ${result.data.openTime / 1000 / 60} 分钟 后重试吧! \n`;
		} else {
			console.log(`\n 宝箱信息:  ${usre_name} 可以宝箱信息,去 宝箱信息 喽! \n`);
			msg += `宝箱信息:  ${usre_name} 可以宝箱信息,去 宝箱信息 喽! \n`;
		...
		msg += `宝箱信息: ${result.error_msg} \n`;
	} else {
		console.log(`\n 宝箱信息: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `宝箱信息: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}
...
async function Share(id) {
	...
	if (result.result == 1) {
		...
		msg += `分享: 获得 金币 ${result.data.amount} 枚!\n`;
        isNotify = true
	} else if (result.result == 14004) {
		console.log(`\n 分享: 今天已经分享过了,明天再来吧!\n`);
		msg += `分享: 今天已经分享过了,明天再来吧!\n\n`;
	} else {
		console.log(`\n 分享: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `分享: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}
