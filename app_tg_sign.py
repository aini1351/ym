...
print(API_ID1)
#session_name = API_ID[:]
CHANNEL_ID = ['@qweybgbot', '@EmbyPublicBot',]
...
async def main(api_id, api_hash, channel_id):
    ...
    async with TelegramClient("id_" + str(api_id), api_id, api_hash) as client:
        ...
        async def my_event_handler(event):
            ...
            if "已经签到过" in event.message.text or "距离下次可签到" in event.message.text or '当前积分' in event.message.text:
            ...
            elif "输入签到验证码" in event.message.text or "输入错误或超时" in event.message.text:  # 获取图像验证码
                ...
                solved_result = await captcha_solver()
                if solved_result:
                    solved_result = solved_result
                else:
                    solved_result = '2b'
                    print_now('识别图片验证码失败，输入2b尝试')
                if len(solved_result) != 2:
