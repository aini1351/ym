...
青龙变量
==========
ttlhd='4787b47b-0199-4ed8-8fde-8c3d2513a297'    多账号@隔开
==========
...
let ttlhds = "",ttlpasswd = '', ttlphone = '', ttlphoneArr = []
const logs =0;
...
await my() 
        ...
        if (ttlhdArr.length === 0) {//无token
        
        if (process.env.ttlphone && process.env.ttlpasswd) { //获取token
            if (process.env.ttlphone && process.env.ttlphone.indexOf('@') > -1) {
            ttlphoneArr = process.env.ttlphone.split('@');
            console.log(`您选择的是用"@"隔开\n`)
        } else {
            ttlphone = [process.env.ttlphone]
        };
        Object.keys(ttlphone).forEach((item) => {
        if (ttlphone[item]) {
            ttlphoneArr.push(ttlphone[item])
        }
        })
        ttlpasswd = process.env.ttlpasswd
        for (let i = 0; i < ttlphoneArr.length; i++) {
            
            await gettoken(ttlphoneArr[i],ttlpasswd,i + 1)
        }
        }
        }
        await $.wait(5000)
          console.log(ttlhdArr)   
...
function gettoken(phone, passwd,i) {
let dat = {'mthd':"login",'platform':"wx_mini",'userName':phone,'password':passwd}
axios.post('https://www.ttljf.com/ttl_chefHub/login/restaurant', dat, {
   headers: {
	"Host": "www.ttljf.com",
	...
	"User-Agent": "Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4 Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/3185 MMWEBSDK/20220105 Mobile Safari/537.36 MMWEBID/8553 MicroMessenger/8.0.19.2080(0x28001337) Process/appbrand0 WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 MiniProgramEnv/android",
...
.then(res=>{
    ...
    console.log('第' + i +'个token为：',res.data.data.token);   
    ttlhdArr.push(res.data.data.token)   
    console.log(ttlhdArr)      
})
}
...
async function blog(){
 return new Promise((resolve) => {
     body = {"id":"D7523371-4C8A-4951-8A99-FCDE066AFEC2","type":"blog"}
     request.put(ttl('Common/share/D7523371-4C8A-4951-8A99-FCDE066AFEC2/blog',body), function(error, response, body) {
try {
        if (error) {
